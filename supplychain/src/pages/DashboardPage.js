  ...
  render() {
    ...
    if (this.state.assetList.length != 0) {
      ...
      for (let i = 0; i < this.state.assetList.length; i++) {
        let date = new Date(
          this.state.assetList[i].time * 1000,
        ...
        asts = (
          <tbody key={i}>
            <tr>
              <th scope="row">{i + 1}</th>
              <td>
                <a href={'/trackAsset/' + this.state.assetList[i].AID}>
                  {this.state.assetList[i].AName}
                </a>
              </td>
              <td>{creator}</td>
              <td>{owner}</td>
      ...
      message = (
        <Typography type="display-4">No assets created or owned!</Typography>
      );
    }
